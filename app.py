...
show_scars     = st.sidebar.checkbox("Landslide Scars", True)
...
with tab1:
    ...
    folium.TileLayer("Stamen Terrain",
                     name="Terrain",
                     attr="Map tiles by Stamen").add_to(m)
    folium.TileLayer(
    ...
    if show_mask:
        add_ee(m, get_mask(), vis['Mask'], 'Mask')
    if show_scars:
        add_ee(m, get_scars(), vis['Scars'], 'Scars')
...
with tab2:
    ...
    # add EE layers:
    if show_hillshade:
        gm.addLayer(get_hillshade(), vis['Hillshade'],
                    "Hillshade", shown=False, opacity=0.5)
    if show_ndvi:
        gm.addLayer(get_ndvi(), vis['NDVI'], "NDVI", shown=False)
    if show_slope:
        gm.addLayer(get_slope(), vis['Slope'], "Slope", shown=False)
    if show_mask:
        gm.addLayer(get_mask(), vis['Mask'], "Mask", shown=True)
    if show_scars:
        gm.addLayer(get_scars(), vis['Scars'], "Scars", shown=True)

    # points via geemap helper
    if show_points:
        pts = get_points().rename(columns={'Longitude':'lon','Latitude':'lat'})
        gm.add_points_from_xy(
            pts, x='lon', y='lat',
...
# -------- Bottom panel: histogram or table --------
if show_points and 'df' in locals() and not get_points().empty:
